...
pub fn bindInsertValues(comptime T: type, st: *Stmt, entity: T) errors.ZiteError!void {
    const ti = @typeInfo(T);
    if (ti != .@"struct") @compileError("bindInsertValues expects a struct type");
...
pub fn bindUpdateValues(comptime T: type, st: *Stmt, entity: T) errors.ZiteError!void {
    const ti = @typeInfo(T);
    if (ti != .@"struct") @compileError("bindUpdateValues expects a struct type");
    const m = comptime meta.getMeta(T);
